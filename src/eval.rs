...
pub fn bcode(bytes: &[u8], pix_buffer: &mut [[u8; 256]; 144], spawned: bool, spawn_index: usize, mut heap: Vec<u8>, mut stack: Vec<u8>, mut routines: Vec<u32>, tx: &Sender<String>) -> (Vec<u32>, Vec<u8>, Vec<u8>) {
    ...
    while &index < &bytes.len() {
        ...
        match byte {
            ...
            0x0c => { // call
                ...
                let evaled = bcode(&bytes, pix_buffer, true, start as usize, heap, stack.clone(), routines.clone(), tx);
                stack = evaled.1;
                heap = evaled.2;
            }
            0x10 => { // end
